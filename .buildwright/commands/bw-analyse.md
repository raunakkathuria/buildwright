...
3. Explore and write `STACK.md`:
   ...
   - Read `package.json`, `Cargo.toml`, `go.mod`, or `pyproject.toml` (whichever exists)
...
4. Explore and write `ARCHITECTURE.md`:
   ...
   - Map directory structure (excluding `node_modules`, `.git`, and build output)
...
10. Report: list four docs with line counts, summarise key findings, and suggest the next step.
...
- [ ] `.buildwright/codebase/` contains `STACK.md`, `ARCHITECTURE.md`, `CONVENTIONS.md`, and `CONCERNS.md`
